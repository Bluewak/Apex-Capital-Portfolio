...
| 채권 BOND | 종합 | 미국 종합채권 `AGG` | **AGG** (BND 제외) | ✅ (R2 잠정 승격) |
| 현금성 CASH | 단기 | 단기채/현금성 `CASH_ST` | SHY / BIL | ✅ |
| 실물 REAL | 귀금속 | 금 `GOLD` | GLD | ✅ |

> MVP 배분 슬롯 = **9종**(`US_LC, US_TECH, DEV_EXUS, EM, GOV_MID, GOV_LONG, AGG, CASH_ST, GOLD`). 배당주는 v2 후보.
> **AGG 승격 사유(2026-07-03 레드팀 R2)**: 안정형 채권밴드 50–70%(§3)를 IEF·TLT만으로는 단일ETF≤30%·`GOV_LONG`≤15% 제약 하에 최대 45%밖에 못 채워 **구성 불가**였음. AGG(듀레이션≈6, 2003-09 상장 → 20년+ 실데이터, splice 불필요)를 채권 코어로 넣어 `IEF 30 + AGG 20 + TLT 10 = 60%`처럼 집중도 위반 없이 밴드 충족. **BND(2007 상장, 19년으로 20년 미달)는 제외.** 잠정 승격이며 최종 확정은 §3·§7 M5 게이트에서 실측 판정.
...
> **안정형 채권밴드 ↔ 05 §3 리스크 상한 정합성 (2026-07-03 레드팀 R2→R3 확정)**: 채권 50–70%를 IEF·TLT·AGG 같은 중기+ 채권으로 채우면 **듀레이션이 강제 적재**되어, 2022처럼 주식·채권이 **동반 하락**하는 국면에선 어떤 long-only·무헤지 안정형(주식 하한 20%)도 실측 MDD가 -10%를 넘긴다(R3 퀀트 실측 ≈-15~-20%). 이는 티커 교체(AGG/SHY)로는 풀 수 없는 07↔05 구조 긴장이었다. **R3 확정 해법("스트레스는 공시, 차단은 평시")**: (1) **산술 구성**은 AGG 승격(§1)으로 채권 코어를 세워 밴드 50–70%를 집중도 위반 없이 충족. (2) **리스크 상한**은 05 §3 R3대로 **평시(위기 3구간 제외) 기준으로 판정**하고, 2008/2020/2022 동반하락은 **차단이 아니라 리포트 "disclosed 스트레스 초과"로 고지**. → 안정형은 정상 발행되며(서비스 불능 회피), 최악 스트레스는 정직하게 공시된다. **B안(채권 하한 45% 하향)·C안(SHY 재분류)은 불필요해져 폐기** — 하한을 낮추거나 무늬만 채권으로 봉합하지 않고, 상한의 판정 기준을 평시로 바로잡아 근본 해결.

> **초안정형(near-cash) 밴드 신설 (2026-07-05 R5, 08 §10)**: Q6=-5% 최보수 고객용 5번째 유형. 성향 4구간 위에 **최하위 티어**로 얹는다. 밴드: **주식 0–10% · 채권(단기 중심, `GOV_LONG` 0) 15–30% · 현금성 `CASH_ST`(SHY) 55–75% · 금 0–10%**. 구성 예시 `SHY 65 / SPY 8 / GLD 8 / (SHY 여분) 19` → σ≈1.9%, VaR95_annual≈3.1%(05 §3 초안정형 상한 -5% 통과). near-cash slot은 SHY 사용(진짜 현금 BIL/SGOV는 20년 미달 → v2 splice). **개인화 배분 아닌 몰개성 "유형 예시"로만 제시** + 기대수익 ~연 2.5%(인플레 미달 가능) 트레이드오프 고지. 실측 발행조건은 §7·M5 게이트(평시 VaR≤-5%).
...
| -- | -- | -- |
| 배당주 `DIV` | 주식 | 인컴 수요 옵션 |
| ~~종합채권 `AGG`~~ | 채권 | **→ MVP로 승격됨(§1, 2026-07-03 R2). 안정형 채권밴드 구성 위해 편입.** |
| 리츠 `REIT` | 실물 | 인플레·인컴 분산 |
| 팩터(모멘텀/퀄리티/저변동) | 주식 | 스타일 틸트 |
| 하이일드 `HY` | 채권 | 신용 팩터 도입(리스크 재정의 필요) |
| 개별종목 | 주식 | v2 규제·데이터 요건 충족 후 → **테마 분류·집중도는 §6** (후보 유니버스: [S&P500 20개년 합집합 840종](../tools/sp500/sp500_by_sector.html)) |
...
[`build_sp500_universe.py`](../tools/sp500/build_sp500_universe.py)가 이 테마 분류를 **구현**한다 — [생성 HTML](../tools/sp500/sp500_by_sector.html) · [README](../tools/sp500/README.md). 2026-07-07 기준 **20개년(2006-07~2026-07)** 합집합 **840종**(현재 503 + 편출 353)을 재분류하며, **현재 503종은 §6.3 크로스워크로 세부테마까지 100% 자동 배정(미매칭 0)**, 편출 353종은 아래 4번 규칙으로 테마군까지 축소 매핑(세부테마 `*.기타`/미분류). 구현 방식:
...
- [x] **유니버스 룩백 vs 백테스트 기간 불일치** (2026-07-07 해소): 도구를 **20개년 합집합(840종, 현재 503 + 편출 353)**으로 확장해 백테스트 20년+(01 §5)과 룩백 정합. Wikipedia 변경이력이 1976~ 담아 20년 편출 포착 가능. 잔여: 편출종목의 point-in-time 정확 편입/편출일·상폐 종목 테마 보강(yfinance 조회 실패분은 미분류)은 v2 정밀화 대상.
