...
> **안정형 채권밴드 ↔ 05 §3 리스크 상한 정합성 (2026-07-03 레드팀 R2, 미해결→M5 판정)**: 채권 50–70%를 IEF·TLT 같은 중·장기채로 채우면 **듀레이션이 강제 적재**되어 2022 금리인상 국면에서 안정형 `MDD ≥ -10%`(05 §3)를 위반할 수 있다. 이는 티커 교체로 산술(구성)과 리스크(상한 통과)를 **동시에** 풀 수 없는 07↔05 설계 긴장이다. **채택 방침**: AGG 승격(§1)으로 산술 구성 blocker를 우선 제거하고, **안정형 4종 포트가 실제로 05 §3 상한(vol≤6%·MDD≥-10%)을 통과하는지는 §7·M5 "포트↔상한 사전검증 게이트"에서 실측으로 판정**한다. 통과 못 하면 **안정형 채권 하한을 45%로 하향(B안, 이때 본 밴드를 05 §3에 맞춰 개정)**. SHY를 채권으로 재분류하는 안(C)은 저듀레이션이라 숫자상 하한은 채우나 방어 듀레이션이 없어 **"무늬만 채권" 착시**로 성향 위반을 은폐하므로 **기각**.
